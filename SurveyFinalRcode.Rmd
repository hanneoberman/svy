...
# Remove 2 cases where a_employ is missing
society <- society[society$a_employ != "don't know" & a_employ != "refuse",]
...
fullmodel <- glm(NR ~ a_gor_dv + a_urban_dv + a_sex + a_racel_dv + 
                   a_employ + a_dvage + a_agegr13_dv + a_mastat_dv + a_livesp_dv +  
                   a_livewith + a_cohab_dv + a_single_dv + a_depchl_dv + a_resp16_dv + 
...
halffullmodel <- glm(NR ~ a_gor_dv + a_urban_dv + a_sex + a_racel_dv + a_employ + a_dvage + 
                       a_agegr5_dv + a_agegr10_dv + a_agegr13_dv + hh_type + hh_size + n_child
                       ,family = binomial, data = society)
