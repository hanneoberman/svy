...
levels(society$a_agegr11_dv) <- c(rep("0-17 years old", 2), "18-19 years old",  "20-24 years old",  "25-29 years old",  "30-34 years old",  
                                  "35-39 years old", "40-44 years old",  "45-49 years old",  "50-54 years old", "55-59 years old",  rep("60 years or older",2))

# update sampling design 
Design <- svydesign(ids=~pidp, strata=~a_strata, data=society, weights=~a_psnenip_xw)

tab <- svytable(~a_agegr11_dv + hh_type, Design) # creates a contingency table of 
...
png("Household composition with age", width = 1200, height = 500, res = 72)
plot(tab, xlab = "age", ylab = "household types", main = "Household types per age category",
     col=c("steelblue1","sienna1","plum2","goldenrod1")
     )
legend("left", c("Couple with children", "Single parent with children", "couple without children", "single without children"), col =c("steelblue1","plum2","sienna1","goldenrod1"), horiz = FALSE )
...
# try to do Chisq test but this doesn't work:  
svychisq(~a_agegr11_dv + hh_type, design=Design, statistic = "Chisq")
#svychisq(~ind+ridreth2, design = nhanes_design, data = nhanes, statistic = "Chisq")
#svychisq(~BMIcu+ridreth2, nhanes_design, statistic = "Chisq")
...
levels(society$a_racel_dv) <- c(rep("missing", 4), "UK Native", rep("White - Nonnative", 2), rep("Mixed", 4), rep("Asian or Asian British", 4), rep("Black/African/Carribean", 3), rep("Other",2))
...
halffullmodel <- glm(NR ~ a_gor_dv + a_urban_dv + a_sex + a_racel_dv + a_employ + a_dvage + 
