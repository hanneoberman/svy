...
coeff <- with(society_3, lm(a_psnenip_xw ~ a_psnenip_xd + a_sex + a_dvage + a_gor_dv))
...
society_3$gor_groups[society_3$a_gor_dv=="london"] <- "London"
society_3$gor_groups[society_3$a_gor_dv == "scotland"] <- "Scotland"
society_3$gor_groups[society_3$a_gor_dv == "west midlands"] <- "West Midlands"

# Change government office region groups and age into factors
society_3$gor_groups <- as.factor(society_3$gor_groups)
society_3$age_fac <- as.factor(society_3$a_dvage)

# Rerun linear regression
coeff2 <- with(society_3, lm(a_psnenip_xw ~ a_psnenip_xd + a_sex + gor_groups + age_fac))
...
# Create model
I_personal <- with(Nonmiss, subset(Design, NR==0))
